...
public class BasicBehaviour : MonoBehaviour
{
    ...
    public float sprintFOV = 100f;                        // the FOV to use on the camera when player is sprinting.
                                                          //public string sprintButton = "Sprint";                // Default sprint button input name.  REMOVED
    ...
    void Update()
    {
        ...
        {
            // Добавить таймаут для автоматической разблокировки
            if (Time.time - lockStartTime > maxLockDuration)
            {
                behaviourLocked = 0;
            }
        }
        ...
        if (IsSprinting() && camScript)
        {
            changedFOV = true;
            camScript.SetFOV(sprintFOV);
        }
        else if (changedFOV && camScript)
        {
            camScript.ResetFOV();
            changedFOV = false;
        }
